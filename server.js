...
function getFeaturedImage(post_id, meta_key){
    ...
    return new Promise((resolve, reject) => {
        ...
        connection.query('SELECT meta_value FROM wp_postmeta WHERE post_id="' + post_id + '" AND meta_key="_thumbnail_id"', (error, results, fields) => {

            if(error) console.log(error);

            let value = results[0].meta_value;

            connection.query('SELECT meta_value FROM wp_postmeta WHERE post_id="'+ value +'" AND meta_key="_wp_attachment_metadata"', (error, results, fields) => {

                if(error) console.log(error);

                resolve(unserialize(results[0].meta_value));
...
app.get('/posts', (req, res) => {
    ...
    connection.query('SELECT ID, post_date, post_title, post_name, post_author, post_excerpt FROM wp_posts WHERE post_type="post" AND post_status="publish"', (error, results, fields) => {
...
app.get('/posts/:post_id/image', (req, res) => {
    ...
    getFeaturedImage(req.params.post_id).then(image => {
        ...
        res.end(JSON.stringify(featuredImage));
    })
